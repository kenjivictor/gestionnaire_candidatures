...
def color_result(val):
    ...
    if val == 'Offre': return 'background-color: #ffff99; font-weight: bold' # Jaune/Or
    return ''
...
if menu_selected == "Ajouter":
    ...
    if isinstance(data, list):
        ...
        if len(data) >0:
            ...
            # Condition A : Candidatures 'En attente'
            cond_attente = (df['resultat'] == 'En attente')

            # Condition B : Si date_reponse est VIDE (NaT), on check date_candidature
            cond_sans_rep = df['date_reponse'].isna() & (df['date_candidature'] < seuil_relance)

            # Condition C : Si date_reponse est REMPLIE, on check date_reponse
            cond_avec_rep = df['date_reponse'].notna() & (df['date_reponse'] < seuil_relance)

            # Application du filtre final (Union des conditions B et C, limitée aux 'En attente')
            a_relancer = df[cond_attente & (cond_sans_rep | cond_avec_rep)]

            # On crée une colonne temporaire pour l'affichage du délai
            df_alert = a_relancer.copy()
            df_alert['jours_attente'] = (datetime.now() - df_alert['date_candidature']).dt.days


            # affichage du message
            if not df_alert.empty:
                st.warning(f"⚠️ **Attention :** Tu as **{len(df_alert)}** candidature(s) sans réponse depuis plus de 15 jours. Il est temps de relancer !")
                with st.expander("🔍 Voir les entreprises à relancer"):
                    st.table(df_alert[['societe_nom', 'titre_poste', 'jours_attente', 'date_candidature', 'societe_mail']])
            else:
                st.success("✅ Toutes tes relances sont à jour !")
        ...
        if len(data) >0:
            ...
            st.dataframe(
                df.style.map(color_result, subset=['resultat']),
                width='content',
                hide_index=True
                )
    ...
    # Petit outil pour ouvrir le PDF sauvegardé
    with st.expander("📂 Consulter une offre sauvegardée"):
        offres_pdf = [f for f in os.listdir(PDF_FOLDER_PATH) if f.endswith('.pdf')]
        if offres_pdf:
            file_to_open = st.selectbox("Choisir un fichier", offres_pdf)
            with open(os.path.join(PDF_FOLDER_PATH, file_to_open), "rb") as f:
                st.download_button("Télécharger/Voir le PDF", f, file_name=file_to_open)
        else:
            st.write("Aucun PDF stocké pour le moment.")
